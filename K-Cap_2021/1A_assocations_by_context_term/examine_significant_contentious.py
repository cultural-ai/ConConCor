...
def main():
    ...
    for min_targets_count in [0,10,20,50]:
        ...
        stats_of_interest = stats.loc[(stats['targets_count']>=min_targets_count) & (stats['contentious_p_value'] < 0.05),:].sort_values('p_con_given_context_token', ascending=False)
        
        # create a string of "token (0.76), token(0.65), ..."
        entries = []
        for count, (index, row) in enumerate(stats_of_interest.iloc[:20,:].iterrows()):
            # entries.append(f"{row['token']}({row['p_con_given_context_token']:.2f})")
            entries.append(f"{row['token']}")

        d['min target count'].append(str(min_targets_count))
        d['tokens significantly associated with contentious samples'].append(", ".join(entries))

    # sav
    df = pd.DataFrame.from_dict(d)
    with open('contentious_terms_various_t.csv', 'w') as f:
        df.to_csv(f)
